...
<ec:table style="" items="items" var="item" retrieveRowsCallback="limit" sortRowsCallback="limit" 
	...
	<ec:row>
	    <ec:column width="30" property="_s" title="√" style="text-align:center">
	    	...
	    	<input type="checkbox" name="selectedItems" id="${item.id}" value="${item.id}" class="checkbox" onchange="saveSel(this)"/>
	    </ec:column>
		...
		<ec:column width="140" property="dept.name" title="部门" styleClass="dept-${item.id}"/>
	...
	<script type="text/javascript">
        ...
        function saveSel(item) {
            if ($(item).is(":checked")) {
                ...
                _selMap()[item.id] = {id:item.id, name:name, deptName:deptName,mobile:mobile};
