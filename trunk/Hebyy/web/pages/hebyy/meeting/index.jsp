	  ...
	  <tr>
	    <td width="70%">
	      <s:form id="pageQueryForm" action="queryxx.do" cssStyle="margin:auto;" method="post">&nbsp;
			...
			会议名称：<s:textfield name="model.hyMc" maxLength="12" cssStyle="width:100px; height:16px;" />&nbsp;
    	          会议纪要类别：<s:select name="model.meetingType.id" list='typeMap' cssStyle="width:100px;" headerKey="" headerValue="请选择" />&nbsp;&nbsp;&nbsp;
			起止时间：<input type="text" name="model.startTime" style="width:90px; height:16px;" readonly="readonly"  value='<s:date name="model.startTime" format="yyyy-MM-dd"/>' onClick="WdatePicker({skin:'blueFresh'})" class="Wdate" />&nbsp;
			截止时间：<input type="text" name="model.endTime" style="width:90px; height:16px;" readonly="readonly"  value='<s:date name="model.endTime" format="yyyy-MM-dd"/>' onClick="WdatePicker({skin:'blueFresh'})" class="Wdate" />
	   ...
	   <a href="toEdit.do?zx=0" title="添加会议信息记录"> 
					...
					<a href="${ctx}/meetingType/index.do" title="会议纪要类别管理"> <img src="${ctx}/images/icons/add.gif">&nbsp;会议纪要类别管理</a>&nbsp;&nbsp;
	    </td>
	  </tr>
	</table>
</div>
			...
			<table id="fineTable" width="800">
				...
				<s:iterator value="items"  var="item" status="st">
					<tr onmousemove="this.className='trOver';"
						...
						<td align="center"><fmt:formatDate value="${item.hyTime}" pattern="yyyy-MM-dd"/></td>
						<td align="center">${item.hyContent}</td>
						<td align="center" valign="middle"><a href="toEdit.do?model.id=${item.id}&zx=1" title="修改会议信息记录"> <img src="${ctx}/images/icons/edit.png"></a>&nbsp;&nbsp;&nbsp;&nbsp;</td>
					</tr>
				</s:iterator>
				<tr>
...
<script type="text/javascript">
	...
	<script type="text/javascript">
		function jkjsSave() {
			$('#pageQueryForm').attr("action", "${ctx}/meeting/toAll.do");
