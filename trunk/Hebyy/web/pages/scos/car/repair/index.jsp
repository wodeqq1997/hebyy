	...
	function remove(id) {
		if (confirm("确认要删除该车辆维修记录？")) {
			window.location.href = URL_PREFIX
					+ "/car/repair/remove.do?model.id=" + id;
		}
	}
</script>
</head>
	...
	<table width="100%">
		<tr>
			<td>维修类型：<s:select list="#{'维修':'维修','保养':'保养'}"
				...
				cssStyle="width:60px;" /> 汽车牌照：<s:textfield id="engineNo"
				name="model.car.carNo" /> 开始日期：<input id="startDate"
				name="startDate"
			...
			<td align="right"><a href="${ctx}/car/repair/edit.do"> <img
				src="${ctx}/images/icons/add.gif">添加车辆维修记录</a>
			&nbsp;&nbsp;&nbsp;&nbsp;</td>
		</tr>
	</table>
</s:form></div>
...
<table id="fineTable" width="800">
	...
	<s:iterator value="#attr.items" var="item" status="st">
		<tr onmousemove="this.className='trOver';"
			...
			<td align="center"><a
				href="${ctx}/car/view.do?model.id=${item.car.id}" target="_blank"><font color="blue">${item.car.carNo}</font></a></td>
			<td align="center">${item.repairType }</td>
			<td><fmt:formatDate value="${item.repairDate}"
