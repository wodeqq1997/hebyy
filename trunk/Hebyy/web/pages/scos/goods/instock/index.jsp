...
function remove(id) {
	if(confirm("确认要删除?")) {
		window.location.href = URL_PREFIX+"/goods/instock/remove.do?model.id=" + id;
	}
}
</script>
</head>
...
<div class="x-panel-header">
	<div style="float: left;">物品入库单管理</div>
</div>
<div class="x-toolbar">
	<table width="100%">
	  <tr>
	...
	<s:hidden name="ec_p" id="ec_p"/>
	  ...
	  <td width="30%" align="right"><a href="${ctx}/goods/instock/edit.do">
		<img src="${ctx}/images/icons/add.gif">&nbsp;添加物品入库单</a>
		&nbsp;&nbsp;
...
<table id="fineTable" width="800">
	...
	<s:iterator value="#attr.items"  var="item" status="st">
		<tr onmousemove="this.className='trOver';"
            ...
            <td align="center"><a href="${ctx}/goods/instock/view.do?model.id=${item.id}" title="${item.stockNo}" target="_blank"><font color="blue">${item.stockNo}</font></a></td>
            <td><a href="${ctx}/goods/view.do?model.id=${item.goods.id}" title="${item.goods.name}" target="_blank"><font color="blue">${item.goods.name}</font></a></td>
