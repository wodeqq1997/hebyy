...
<body>
	...
	<div id="r_main">
		<div class="x-header" style="border-bottom: 1px solid #99bbe8;" >
			<span
				class="r_nav_item">借书还书记录查询</span>
		</div>
	  ...
	  <tr>
	    <td width="70%">
	      	<s:form id="pageQueryForm" action="queryxx.do" cssStyle="margin:auto;" method="post">&nbsp;
					...
					书名：<s:textfield name="model.bookMc"
						maxLength="12" cssStyle="width:100px; height:16px;" />
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					借书人：<s:textfield name="model.jsr"
						maxLength="12" cssStyle="width:100px; height:16px;" />
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;						
				          是否归还：<s:select name="model.isgh" maxLength="5" list="{'已归还','未归还'}"
				    ...
				    <input type="button" onclick="jkjsSave()"  class="button" 
						value=" 查询全部" style="width:70px; height:18px;">
						  ...
						  <a href="${ctx}/tsgl/queryxx1.do" title="返回"> 
			...
			<table id="fineTable" width="800">
				...
				<s:iterator value="jhList"  var="item" status="st">
					<tr onmousemove="this.className='trOver';"
						...
						<td  width="155" align="center"><s:if test='#attr.item.hsbz == null'>- -</s:if> ${item.hsbz}
						</td>
						
								<td align="center" valign="middle"><s:if test='#attr.item.hsr == null'>
								<input type="button"
						onclick="javascript:window.location.href='toHs.do?item.id=${item.id}&&item.bookId=${item.bookId}';"
						class="button" value=" 归还" >
								
								</s:if>
								<s:if test='#attr.item.hsr != null'>
								<input type="button"
						onclick="javascript:window.location.href='toHs.do?item.id=${item.id}&&item.bookId=${item.bookId}';"
