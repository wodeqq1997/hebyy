...
<table id="fineTable" width="800">
	...
	<s:iterator value="items" var="item" status="st">
		<tr onmousemove="this.className='trOver';"
			...
			<td width="33" align="center"><input type="checkbox" name="selectedItems" id="selectedItems" value="${item.id}" class="checkbox" /></td>
			<td width="100" align="center"><a href="view.do?model.id=${item.id}" title="查看详情">
			<font color="blue">${item.tsMc}</font></a></td>
			<td width="75" align="center">${item.author}</td>
			<td width="85" align="center">${item.lbxx.lbMc}</td>
			<td width="65" align="center">${item.totalNum}</td>
			<td width="70" align="center">
			<s:if test="#attr.item.syNum == 0"><font color="red">${item.syNum}</font></s:if>
			<s:if test="#attr.item.syNum != 0">${item.syNum}</s:if>
			</td>
			<td width="80" align="center"><fmt:formatDate value="${item.rkTime}" pattern="yyyy-MM-dd" /></td>
			<td width="150" align="center">${item.descn} <input type="hidden" name="item.lbxx.lbMc" value="${item.lbxx.lbMc}" /></td>
			<td align="center" valign="middle"><a href="toEdit.do?model.id=${item.id}&&zx=1" title="修改图书信息"> <img src="${ctx}/images/icons/edit.png"> </a>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		</tr>
	</s:iterator>
	<tr>
