...
function remove(id){
	if (confirm("确认要删除该案卷类别吗?")){
		window.location.href=URL_PREFIX+"/archive/type/remove.do?model.id=" + id;
	}
}
</script>
</head>
...
<div class="x-panel-header" >
	<div style="float: left;">案卷类别管理</div>
</div>
...
<div class="x-toolbar">
        ...
        <s:hidden name="ec_p" id="ec_p"/>
			<table width="99%">
				<tr>
					<td width="70%">
						案卷类别名称：<s:textfield id="name" name="model.name"  cssStyle="height:19px;" />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="查询" class="button" />
					</td>
					<td width="30%" align="right" >
						<a href="${ctx}/archive/type/edit.do"><img src="${ctx}/images/icons/add.gif">添加案卷类别</a>
						&nbsp;
...
<table id="fineTable" width="800" align="left">
	...
	<s:iterator value="#attr.items"  var="item" status="st">
		<tr onmousemove="this.className='trOver';"
            ...
            <td>${item.name }</td>
			 <td>${item.archiveNumMark}</td>
			<td>${item.remark }</td>
			<td width="125" align="center"><a href="view.do?model.id=${item.id}">查看</a>&nbsp;&nbsp;|&nbsp;&nbsp;
